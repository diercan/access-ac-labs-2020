...
namespace Domain.Domain.EmployeeRoles.ChangeMenuItemOp
{
    public class ChangeMenuItemOp : OpInterpreter<ChangeMenuItemCmd, ChangeMenuItemResult.IChangeMenuItemResult, Unit>
    {
        ...
        {
            return !ExistsChanges(Op.CurrentMenuItem, Op.NewMenuItem) ?
                Task.FromResult<IChangeMenuItemResult>(new MenuItemNotChanged("No any changes! The new mwnu item is the same with the current menu item!")) :
                Task.FromResult<IChangeMenuItemResult>(new MenuItemChanged(Op.NewMenuItem));
        }
