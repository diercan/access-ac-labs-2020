...
namespace Domain.Domain.GetClientOp
{
    ...
    {
        private List<Client>.Enumerator c = App.ClientsList.GetEnumerator();    // c is enumerator for ClientsList
        ...
        public override Task<IGetClientResult> Work(GetClientCmd Op, Unit state)
        {
            ...
            return !Exists(Op.ClientId) ?
                ...
                Task.FromResult<IGetClientResult>(new GetClientSuccessful(new ClientAgg(c.Current)));
        }
        ...
        public bool Exists(string clientId)
        {
            while (c.MoveNext())
            {
                if (clientId.CompareTo(c.Current.Id) == 0)
                {
                    return true;
                }
            }
